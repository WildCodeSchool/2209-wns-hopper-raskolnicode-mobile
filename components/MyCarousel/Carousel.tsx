        ...
        {data?.getBlogs?.map((itemData) => {
          return (
            <View style={styles.card} key={itemData.id}>
              <Pressable
                onPress={() =>
                  navigation.navigate(`Blog`, { itemId: itemData.id })
                }
              >
                {itemData?.picture ? (
                  <Image
                  ...
                  <Image
                    style={styles.image}
                    source={require("../../assets/default-post-img.png")}
                  />
                )}
                <Text style={styles.title}>
                  {truncateText(itemData?.name, 15)}
                </Text>
                <Text style={styles.description}>{truncateText(itemData?.description, 70)}</Text>
              </Pressable>
            </View>
          );
        })}
